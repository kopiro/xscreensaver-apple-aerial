...
command -v cvlc >/dev/null 2>&1 || {
  echo "cvlc is not installed" >&2
  exit 1
}

STORAGE_DIR=/opt/apple-aerial
STREAM_HOST="http://sylvan.apple.com/Aerials/2x/Videos"
...
day_db=$XDG_CONFIG_HOME/.apple-aerial-day
night_db=$XDG_CONFIG_HOME/.apple-aerial-night
...
download() {
  [ ! -f "$STORAGE_DIR/$1" ] && wget "$STREAM_HOST/$1" -O "$STORAGE_DIR/$1"
}

if [ "$1" == "--download" ]; then
  for v in "${DayArr[@]}"; do download "$v"; done
  for v in "${NightArr[@]}"; do download "$v"; done
  exit 0
fi

if [ "$1" == "--download-day" ]; then
  for v in "${DayArr[@]}"; do download "$v"; done
  exit 0
fi

if [ "$1" == "--download-night" ]; then
  for v in "${NightArr[@]}"; do download "$v"; done
  exit 0
fi
...
do
  ...
  if [[ -f "$fs_video" ]]; then
    ...
    echo "The chosen file hasn't been downloaded, skipping."
  fi
done
